...
describe('resource config', () => {
  test('retrieve', async () => {
    const responsePromise = openTransit.config.retrieve();
  ...
  test('retrieve: request options instead of params are passed correctly', async () => {
    ...
    await expect(openTransit.config.retrieve({ path: '/_stainless_unknown_path' })).rejects.toThrow(
  ...
  test('retrieve: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(
      openTransit.config.retrieve({ key: 'string' }, { path: '/_stainless_unknown_path' }),
